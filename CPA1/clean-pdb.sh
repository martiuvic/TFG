...
grep -v BGLU <$thing_clean | grep -v BHIS | grep -v GOL | grep -v BLEU | grep -v BPRO | grep -v BLYS | grep -v BSER | grep -v BLEU | grep -v BASP | grep -v BPHE | grep -v BILE | grep -v CSER | grep -v BVAL | sed 's/AGLU/ GLU/g' | sed 's/ASER/ SER/g' | sed 's/APRO/ PRO/g' | sed 's/ALYS/ LYS/g' | sed 's/AVAL/ VAL/g' | sed 's/APHE/ PHE/g' | sed 's/AHOH/ HOH/g' | sed 's/AHIS/ HIS/g' | sed 's/AASP/ ASP/g' | sed 's/ALEU/ LEU/g' | sed 's/AILE/ ILE/g' | sed 's/HOH/WAT/g'

############################# the following lines didn't work properly ################################

# ## Remove any B chain lines
# ## OSX needs to be sed -i "" BLAH to work
# ## Linux use sed -i '/BALA/d' $thing_clean
